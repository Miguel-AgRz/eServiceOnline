...
{
    public class CancelProductHaulIntegrationWithDRBTestForBlend : IntegrationTestBase
    {
        ...
        public void CancelScheduleRigJobBlendProductHaul(int productHaulId, bool cancelShippingLoadSheet)
        {
            ...
            foreach (ShippingLoadSheet shippingLoadSheet in productHaul.ShippingLoadSheets)
            {
                ...
                {
                    IsChecked = cancelShippingLoadSheet,
        ...
        public void TestCancelProductHaulbyLoadRequested()
        {
            ...
            CancelScheduleRigJobBlendProductHaul(productHaul.Id,true);
        ...
        {
            //Create separate Crew information for this test
            int crewId = PrepareSanjelCrewData("CancelCalled");
        ...
        [Test]
        public void TestCancelProductHaulHasAailable()
        {
            //Create separate Crew information for this test
            int crewId = PrepareSanjelCrewData("HasAailable");
            ...
            //Scheduled a ProductHaul and check if the creation was successful
            ScheduleRigJobBlendProductHaul(false, false, crewId);
            ProductHaul preProductHaul = eServiceOnlineGateway.Instance.GetProductHaulByCrew(crewId).FirstOrDefault();
            Assert.NotNull(preProductHaul);
            Assert.AreNotEqual(preProductHaul.Id, 0);



            //Scheduled a ProductHaul and check if the creation was successful
            ScheduleRigJobBlendProductHaul(false, false, crewId);
            ProductHaul productHaul = eServiceOnlineGateway.Instance.GetProductHaulByCrew(crewId)
                .Where(p => p.Id != preProductHaul.Id).FirstOrDefault();
            Assert.NotNull(productHaul);
            Assert.AreNotEqual(productHaul.Id, 0);
            ...
            CancelScheduleRigJobBlendProductHaul(productHaul.Id, false);
        ...
        [Test]
        public void TestCancelProductHaulHasReturned()
        {
            //Create separate Crew information for this test
            int crewId = PrepareSanjelCrewData("HasReturned");
            ...
            bulkerCrewLog.CrewStatus = BulkerCrewStatus.Returned;
            eServiceOnlineGateway.Instance.UpdateBulkerCrewLog(bulkerCrewLog);
